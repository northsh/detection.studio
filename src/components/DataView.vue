...
import { PlusIcon, SearchIcon, AlertCircleIcon, LoaderIcon, CheckCircleIcon } from "lucide-vue-next";
        ...
        <template v-else>
            ...
            <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-2">
                    ...
                    <!-- Error indicator -->
                    <Badge v-else class="bg-red-600 hover:bg-red-700 cursor-pointer" 
                    ...
                    <div v-if="showErrorDetails && sigma?.data_loading_error" 
                        ...
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <button @click="showErrorDetails = false" class="text-white hover:text-red-200">×</button>
                        </div>
            ...
            <Tabs v-model="activeTab" class="flex-1 min-h-0 flex flex-col">
                ...
                <!-- Summary Tab -->
                <TabsContent value="summary" class="flex-1 min-h-0 data-[state=active]:flex data-[state=active]:flex-col">
                    <Card class="p-4">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="font-medium">{{ sigma?.search_results.value?.stats?.totalRecords || 0 }}</span>
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-muted-foreground">File Size</span>
                                        <span class="font-medium">{{ formatBytes(data?.current_data_frame?.length || 0) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="font-medium">{{ sigma?.search_results.value?.stats?.totalMatches || 0 }}</span>
                                    </div>
                                    <div class="flex flex-col">
                    ...
                    <!-- Status messages -->
                    <div v-if="sigma?.search_error" class="mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-md">
                    ...
                    <div v-else-if="sigma?.data_loading_error" class="mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-md">
                        <div class="flex items-center gap-2 text-red-500">
                            <AlertCircleIcon class="h-4 w-4" />
                    ...
                    <div v-else-if="sigma?.is_searching" class="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-md">
                ...
                <!-- Matches Tab -->
                <TabsContent value="matches" class="flex-1 min-h-0 data-[state=active]:flex data-[state=active]:flex-col">
                    <div v-if="sigma?.search_results.value?.matches?.length" class="flex-1 min-h-0 overflow-hidden">
                        <!-- TanStack Table -->
                        <div class="flex flex-col flex-1 min-h-0">
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full text-sm">
                                        <thead class="bg-muted/50">
                                            <tr>
                                                <th v-for="header in tableInstance.getHeaderGroups()[0].headers" :key="header.id" 
                            ...
                            <div class="flex justify-between items-center py-2">
                                <div class="text-xs text-muted-foreground">
                                    ...
                                    to {{ Math.min((tableInstance.getState().pagination.pageIndex + 1) * tableInstance.getState().pagination.pageSize, 
                    ...
                    <!-- No matches state -->
                    <div v-else-if="sigma?.is_data_loaded && !sigma?.is_searching" class="flex flex-col items-center justify-center py-12">
                        <SearchIcon class="h-12 w-12 text-muted-foreground/50 mb-4" />
                        <h3 class="text-lg font-medium">No Matches Found</h3>
                        <p class="text-sm text-muted-foreground mt-1">The current Sigma rule doesn't match any logs in your dataset.</p>
                    </div>
                    
                    <!-- Loading state -->
                    <div v-else-if="sigma?.is_searching" class="flex flex-col items-center justify-center py-12">
                ...
                <!-- Raw Data Tab -->
                <TabsContent value="raw" class="flex-1 min-h-0 data-[state=active]:flex data-[state=active]:flex-col">
