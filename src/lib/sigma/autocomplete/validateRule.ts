    ...
    try {
        ...
        if (yamlContent.detection) {
            ...
            if (!hasSearchIdentifier) {
                result.valid = false;
                result.errors.push(
                    "At least one search identifier is required in the detection section",
                );
            }
            ...
            if (condition && typeof condition === "string") {
                ...
                for (const id of conditionIdentifiers) {
                    if (!searchIdentifiers.includes(id)) {
                        result.valid = false;
                        result.errors.push(
                            `Condition references undefined search identifier: ${id}`,
                        );
                    }
                }
            }
        ...
        if (
            yamlContent.status &&
            !["stable", "test", "experimental", "deprecated", "unsupported"].includes(
                yamlContent.status,
            )
        ) {
            result.valid = false;
