...
describe('SigmaConverter', () => {
  ...
  describe('Conversion', () => {
    ...
    it('should handle worker API exceptions', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error during Pyodide Sigma conversion:', expect.any(Error));
      consoleSpy.mockRestore();
    });
  ...
  describe('Concurrent Operations', () => {
    ...
    it('should handle concurrent conversions', async () => {
      const promises = Array.from({ length: 3 }, (_, i) =>
        converter.convert(`rule${i}`, 'splunk')
      );
    ...
    it('should handle concurrent backend installations', async () => {
      ...
      const promises = Array.from({ length: 3 }, () =>
        converter.convert('rule', 'elastic')
      );
